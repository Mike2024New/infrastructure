...
def process_history_block(
        content: str, version: str, commit_hash: str, history_header: str = '## История развития модуля'
) -> str | None:
    start_history = False
    ...
    for line in content.splitlines():
        ...
        if re.match(pattern=r'^\d{2}\.\d{2}\.\d{4}', string=line):
    ...
    new_block = [
        '<details>',
        f"  <summary>{blocks[0]} - v{version} - {commit_hash}</summary>",
        '\n'.join(blocks[1:]),
        '</details>\n'
    ]
...
def update_module_history(
        md_files_dir: Path, version: str, commit_hash: str, history_header: str = '## История развития модуля',
) -> None:
    """
